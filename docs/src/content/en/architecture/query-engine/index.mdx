...
- [Context Assembly](./context-assembly) — How system prompt, tools, and user context are assembled into a single API call
- [Streaming Pipeline](./streaming-pipeline) — Token-by-token streaming, event dispatch, and stop reason handling
- [Tool Call Loop](./tool-call-loop) — The complete permission → execute → collect → re-query cycle
- [Error Recovery](./error-recovery) — API error handling, rate limiting, retry logic, and tool failure recovery
